  ...
  if (!confirming) {
    return (
      <button
        ...
        className="min-h-11 rounded-full border border-destructive text-destructive px-5 text-sm font-medium"
      >
    ...
    <div className="space-y-3 text-sm">
      ...
      <div className="flex gap-3">
        <button
          ...
          className="min-h-11 rounded-full bg-destructive px-5 font-medium text-white disabled:opacity-50"
        >
          {busy ? 'Envoi…' : 'Confirmer la suppression'}
        </button>
        <button type="button" onClick={() => setConfirming(false)} className="min-h-11 rounded-full border border-border px-5 font-medium">
          Annuler
        </button>
      </div>
      {error && <p className="text-destructive">{error}</p>}
