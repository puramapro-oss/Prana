...
export default function CookieConsentBanner({ appName, politiqueHref = '/politique-confidentialite', onConsent }: CookieConsentBannerProps) {
  ...
  return (
    <div
      ...
      className="fixed inset-x-0 bottom-0 z-50 border-t border-border bg-background/95 backdrop-blur-xl p-4 sm:p-6"
    >
      <div className="mx-auto max-w-4xl space-y-4">
        <p className="text-sm text-foreground">
          ...
          <a href={politiqueHref} className="underline text-primary">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            onClick={acceptAll}
            className="min-h-11 rounded-full bg-primary px-5 text-sm font-medium text-white"
          >
            Tout accepter
          </button>
          <button
            ...
            onClick={refuseAll}
            className="min-h-11 rounded-full border border-border px-5 text-sm font-medium"
          >
            Tout refuser
          </button>
          {customizing ? (
